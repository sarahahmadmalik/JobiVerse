  ...
  return (
    <div className="bg-white p-3 sm:p-4 h-auto sm:h-[360px] rounded-lg sm:rounded-[16px] border border-[#00000024] w-full sm:min-w-60 flex-1 lg:max-w-[400px] overflow-y-auto flex flex-col">
      <div className="flex justify-between items-center mb-3 sm:mb-4">
        ...
        {events.length > 5 && (
          <Link href="/interviews" className="text-blue-600 hover:text-blue-800 text-sm flex items-center gap-1">
            View All
            <svg
              className="w-4 h-4"
              fill="none"
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full text-center py-2 flex-grow">
          ...
          <p className="text-gray-500 text-base sm:text-lg">No upcoming interviews</p>
          <p className="text-gray-400 text-xs sm:text-sm mt-1">Schedule new interviews to see them here</p>
        </div>
      ) : (
        <>
          <ul className="space-y-3 sm:space-y-4 flex-grow">
            {events.slice(0, 5).map((event, index) => {
              // Extract month and day from formatted time if available
              const monthDay = event.time.match(/^[A-Za-z]{3} \d{1,2}/)?.[0] || '';
                ...
                >
                  {month && day ? (
                    ...
                    <div className="min-w-[45px] sm:min-w-[52px] text-center flex flex-col">
                      ...
                      <div className="bg-white border border-blue-100 rounded-b-lg py-0.5 sm:py-1">
                        <span className="text-lg sm:text-md font-bold text-gray-700">
                          {event.time.includes('Today') ? format(new Date(), 'd') : format(new Date().setDate(new Date().getDate() + 1), 'd')}
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1 sm:gap-2">
                      <h4 className="font-medium text-sm sm:text-sm text-gray-800">
                        {event.type} with {event.candidate}
                      </h4>
                      {event.priority === 1 && (
          ...
          {events.length > 5 && (
            <div className="mt-3 text-center">
              <Link href="/interviews" className="text-blue-600 hover:text-blue-800 text-sm inline-flex items-center">
                View More
                <svg
