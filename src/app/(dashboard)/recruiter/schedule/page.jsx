...
import { ChevronLeft, ChevronRight, Video, MapPin, Users, Clock } from "lucide-react";
    ...
    const fetchInterviews = async () => {
      try {
        ...
        const formattedEvents = interviews.map(interview => ({
          id: interview._id,
          title: interview.jobDetails?.jobTitle || `Interview with ${interview.candidates?.[0]?.firstName}`,
          start: new Date(interview.startTime),
          end: new Date(interview.endTime || new Date(new Date(interview.startTime).getTime() + 60 * 60000)),
          ...
          resource: {
            type: getInterviewType(interview.jobDetails?.jobTitle).toLowerCase(),
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <div className="space-y-4">
          ...
          {selectedEvent.resource.candidate && (
            <div className="flex items-start">
              ...
              <div>
                ...
                <p>{selectedEvent.resource.candidate.firstName} {selectedEvent.resource.candidate.lastName}</p>
              </div>
            </div>
          )}
          ...
          {selectedEvent.resource.jobDetails && (
            <div className="flex items-start">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 mt-1 text-gray-500">
...
const upcomingEvents = events
    ...
    .map(event => ({
      id: event.id,
      candidate: event.resource.candidate?.firstName + ' ' + event.resource.candidate?.lastName,
      role: event.resource.jobDetails?.jobTitle || "Position",
      time: formatInterviewTime(event.start),
      type: event.resource.type || getInterviewType(event.resource.jobDetails?.jobTitle),
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex justify-between items-center p-4 bg-white border-b">
            <div className="flex gap-2">
              <button
                onClick={() => handleNavigate(new Date(date.setDate(date.getDate() - 1)))}
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1 rounded text-sm ${
                  currentView === Views.DAY ? "bg-blue-100 text-blue-700" : "hover:bg-gray-100"
                }`}
              >
                Day
              </button>
              <button
                ...
                className={`px-3 py-1 rounded text-sm ${
                  currentView === Views.WEEK ? "bg-blue-100 text-blue-700" : "hover:bg-gray-100"
                }`}
              >
                Week
              </button>
